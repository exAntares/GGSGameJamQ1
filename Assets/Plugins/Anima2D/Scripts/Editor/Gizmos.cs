				...
				if(control && control.isActiveAndEnabled && IsVisible(control.gameObject))
				{
					...
					if(Selection.activeTransform == transform)
					{
						   ...
						   (control.bone && !Selection.transforms.Contains(control.bone.transform)))
						{
                            ...
                            float size = HandleUtility.GetHandleSize(transform.position) * (Selection.activeTransform != transform ? 0.2f : 5.0f);
							...
							}else if(Tools.current == Tool.Rotate)
							{
                                ...
                                var circlesAmmount = 40.0f;
                                for (int i = 0; i < circlesAmmount; i++)
                                {
                                    newRotation = Handles.Disc(newRotation, transform.position, transform.forward, size * i / circlesAmmount, false, 0f);
                                }
                            ...
                            else if (Tools.current == Tool.Scale)
                            {
                                ...
                                var newScale = Handles.ScaleHandle(transform.localScale, transform.position, transform.rotation, size);
                                ...
                                if (EditorGUI.EndChangeCheck())
                                {
                                    ...
                                    if (control.bone)
                                    {
                                        Undo.RecordObject(control.bone.transform, "Scale");

                                        control.bone.transform.localScale = newScale;

                                        EditorUpdater.SetDirty("Scale");
                                    }
                                }
                            }
                        }
