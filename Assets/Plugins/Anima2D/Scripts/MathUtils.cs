...
namespace Anima2D 
{
	public static class MathUtils
	{
		...
		public static float RoundBasedOnMinimumDifference(float valueToRound, float minDifference)
		{
			...
			return (float)System.Math.Round((double)valueToRound, MathUtils.GetNumberOfDecimalsForMinimumDifference(minDifference), MidpointRounding.AwayFromZero);
		}

		public static float DiscardLeastSignificantDecimal(float v)
		{
			int digits = Mathf.Clamp((int)(5f - Mathf.Log10(Mathf.Abs(v))), 0, 15);
			return (float)System.Math.Round((double)v, digits, MidpointRounding.AwayFromZero);
		}
